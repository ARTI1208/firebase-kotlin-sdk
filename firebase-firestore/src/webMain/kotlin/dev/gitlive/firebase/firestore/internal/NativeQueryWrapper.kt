...
internal actual open class NativeQueryWrapper internal actual constructor(actual open val native: NativeQuery) {
    ...
    actual suspend fun get(source: Source) = rethrow { QuerySnapshot(js.get(source).jsAwait()) }

    actual fun limit(limit: Number) = query(
        js,
        dev.gitlive.firebase.firestore.externals.limit(limit.toInt().toJsNumber()),
    ).wrapped
    ...
    private fun Filter.toQueryConstraint(): QueryConstraint = when (this) {
        ...
        is Filter.Field -> {
            ...
            dev.gitlive.firebase.firestore.externals.where(field, constraint.filterOp, value as JsAny?)
        }
        is Filter.Path -> {
            val value = when (constraint) {
                is WhereConstraint.ForNullableObject -> constraint.safeValue
            ...
            dev.gitlive.firebase.firestore.externals.where(path.js, constraint.filterOp, value as JsAny?)
        }
    }
    ...
    actual fun orderBy(field: String, direction: Direction) = rethrow {
        query(js, dev.gitlive.firebase.firestore.externals.orderBy(field, direction.jsString.toJsString())).wrapped
    }
    ...
    actual fun startAfter(vararg fieldValues: Any) = rethrow {
        query(
            js,
            dev.gitlive.firebase.firestore.externals.startAfter(*(fieldValues as Array<JsAny>)),
        ).wrapped
    }
    ...
    actual fun startAt(vararg fieldValues: Any) = rethrow {
        query(
            js,
            dev.gitlive.firebase.firestore.externals.startAt(*(fieldValues as Array<JsAny>)),
        ).wrapped
    }
    ...
    actual fun endBefore(vararg fieldValues: Any) = rethrow {
        query(
            js,
            dev.gitlive.firebase.firestore.externals.endBefore(*(fieldValues as Array<JsAny>)),
        ).wrapped
    }
    ...
    actual fun endAt(vararg fieldValues: Any) = rethrow {
        query(
            js,
            dev.gitlive.firebase.firestore.externals.endAt(*(fieldValues as Array<JsAny>)),
        ).wrapped
    }
