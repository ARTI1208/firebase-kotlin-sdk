...
internal actual class NativeFirebaseFirestoreWrapper internal constructor(
    ...
    internal actual constructor(native: NativeFirebaseFirestore) : this(
        { settings ->
            settings?.let {
                NativeFirebaseFirestore(initializeFirestore(native.js.app, settings.js))
            } ?: native
        },
    )
    ...
    actual suspend fun <T> runTransaction(func: suspend NativeTransaction.() -> T) =
        rethrow {
            dev.gitlive.firebase.firestore.externals.runTransaction(
                js,
                { GlobalScope.promise { NativeTransaction(it).func() as JsAny?} },
            ).jsAwait<JsAny?>() as T
        }

    actual suspend fun clearPersistence() =
        rethrow { clearIndexedDbPersistence(js).jsAwait<Unit>() }
    ...
    actual suspend fun disableNetwork() {
        rethrow { dev.gitlive.firebase.firestore.externals.disableNetwork(js).jsAwait<Unit>() }
    }

    actual suspend fun enableNetwork() {
        rethrow { dev.gitlive.firebase.firestore.externals.enableNetwork(js).jsAwait<Unit>() }
    }
}
