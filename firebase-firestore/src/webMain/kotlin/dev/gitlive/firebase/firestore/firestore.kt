...
public actual data class FirebaseFirestoreSettings(
    ...
    internal val js: Json
        get() = json().apply {
        set("ssl", sslEnabled.toJsBoolean())
        set("host", host.toJsString())
        set(
            "localCache",
            when (cacheSettings) {
                ...
                is LocalCacheSettings.Memory -> {
                    ...
                    memoryLocalCache(json("garbageCollector" to garbageCollectorSettings) as MemoryCacheSettings)
                }
            },
        )
    }
...
internal fun entriesOf(jsObject: JsAny?): List<Pair<String, Any?>> =
    JsObject.entries(jsObject)

// from: https://discuss.kotlinlang.org/t/how-to-access-native-js-object-as-a-map-string-any/509/8
internal fun mapOf(jsObject: JsAny?): Map<String, Any?> =
